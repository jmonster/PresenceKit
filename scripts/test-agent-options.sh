...
"$APP" --check-config
"$APP" --check-config --media '/a path/movie.mp4' --manage-display
...
for bad in '--garbage' '--media' '--absence-seconds NaN' '--absence-seconds 1e300' '--absence-seconds inf' '--absence-seconds -1' '--human --face' '--human --absence-seconds 10' '--fallback guess' '--fallback-grace-seconds 3601'; do
...
echo 'All 14 agent argument checks passed without camera or display side effects.'
