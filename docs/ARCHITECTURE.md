...
## Supervised host integration

`PresenceAutomation` composes (rather than changes) the single-run monitor. It owns one attempt, a bounded ordered event consumer and a tolerant policy timer. Each attempt awaits its lease/inference teardown before capped monotonic backoff. `SensingHealth` earns reset time only across fresh analysis, preserving gaps and semantic availability. Typed transport failures, not diagnostic strings, determine retry. Thread-safe latest-policy/availability snapshots let queued effects reject stale decisions before execution; raw ordered sensing events remain available.

`PresencePlaybackController` drives an injected public `PresencePlaybackOutput`. It owns one cancellable vacancy deadline, suppresses feedback before illumination changes, and invalidates output before slow cleanup. The native output keeps AVPlayer work on MainActor and confines IOKit/subprocess operations to a utility queue. A sleep command receives a revocable permit and is drained on cancellation. `PresenceLifecycle` serializes suspension/resumption across independent reasons without polling or overlapping monitoring tasks. `PresencePlayerController` and the reference agent share these implementations; they do not maintain independent retry loops.
