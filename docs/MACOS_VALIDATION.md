...
Candidate run 35409672667 completed both Linux compiler lanes successfully but failed native compilation: `AVError.Code.mediaServicesWereReset` is not imported by the macOS SDK. The native classifier now uses only supported named macOS codes, leaves unknown codes terminal, and preserves explicit typed reset reasons from custom sources. Native regressions cover both the unknown-code fail-closed path and the portable reset contract without referencing the unavailable symbol. This failure is not treated as a successful native check; the corrected source must pass the complete gate before merge/publication.
